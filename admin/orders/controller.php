...
function doInsert(){
	...
	function doEdit(){
		...
		if ($_GET['actions']=='confirm') {
							# code...
				$status	= 'Confirmed';	
				$remarks ='Pesanan Anda telah dikonfirmasi.';
				$delivered = Date('Y-m-d');

		}elseif ($_GET['actions']=='deliver') {
							# code...
				$status	= 'Delivered';	
				$remarks ='Pesanan Anda telah terkirim.';
				$delivered = Date('Y-m-d');
  	...
  	// 		$res = $customer->single_customer($_GET['customerid']); 
	        ...
	        $sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '".$cur->PHONE."','Janno','FROM kelompok 7 : Your order has been '".$status. "'. The amount is '".$cur->PAYMENT. "')";
	        $mydb->setQuery($sql);
	        $mydb->executeQuery();
			...
			foreach ($cur as $result) {
	  		 ...
	  		 $sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '".$result->OWNERPHONE."','Janno','FROM kelompok 7 : Your  product has been ordered'. The amount is '".$result->PAYMENT. "')";
	        $mydb->setQuery($sql);
	        $mydb->executeQuery();
